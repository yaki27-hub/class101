...
export async function POST(req: Request): Promise<Response> {
  ...
  /*
   * 외형 고정을 최우선 지시로 둔다 — 실사용 제보: 러시안블루가 회백색 이색(오드아이)
   * 고양이로 나왔다. 품종명을 명시하고, 사진에 없는 특징을 더하지 말라고 못 박는다.
   */
  const prompt =
    `이 사진 속 고양이를 따뜻한 손그림 동화풍(cottagecore) 일러스트로 그려줘. ` +
    `가장 중요한 규칙: 사진 속 실제 고양이와 같은 고양이로 알아볼 수 있어야 해. ` +
    `털색·무늬·눈 색을 사진 그대로 유지하고, 사진에 없는 특징(오드아이, 흰 반점, ` +
    `다른 털색)을 새로 만들지 마. 품종은 ${body.breedGroup}이야. ` +
    `아늑한 통나무집 안에서 니트 스웨터를 입고 벽난로 불빛 아래 편안히 쉬는 모습으로. ` +
    `${age.stageLabel} 고양이의 분위기. 포근한 가을 색감, 부드러운 붓터치, 세로 초상화.`;
