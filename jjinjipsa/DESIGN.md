...
**라벨이 있는 버튼은 예외 없이 `rounded-button`(10px) 하나만 쓴다.** 같은 화면에서
pill과 사각 버튼이 섞이면 즉시 어색해 보이므로, 크기·중요도와 무관하게 통일한다.
0px 각진 모서리는 쓰지 않는다 — 10px과 섞이면 시스템이 무너진다.

`rounded-full`은 **형태 자체가 원/알약인 것**에만 허용한다:

| 용도 | 라운드 | 예 |
|---|---|---|
| 모든 버튼 (주·보조·작은 액션) | `button` 10px | 등록하기, 저장, 취소, 수정, 메모, 삭제 |
...
판단 기준: **액션이면 버튼(10px), 선택 대상이면 칩(full).** "수정·메모·제거"는 작지만
액션이므로 버튼이다.
